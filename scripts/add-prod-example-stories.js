...
/**
 * Script to add the 8 example stories to PRODUCTION database
...
// Sanity client for production dataset
const sanityClient = createClient({
  projectId: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || 'xh75mh7d',
  dataset: 'production', // Force production dataset
...
async function syncToSanity(story, username, categories) {
  try {
    ...
    console.log(`  ✓ Synced to Sanity production: ${result._id}`);
    return result;
  } catch (error) {
...
async function addProductionExampleStories() {
  console.log('🌟 Adding example stories to PRODUCTION database...\n');
  console.log('⚠️  This will add stories to the PRODUCTION environment!');
  ...
  try {
    for (let i = 0; i < stories.length; i++) {
      ...
      // Sync to Sanity production
      await syncToSanity(story, storyData.username, storyData.categories);
    }

    console.log('\n✅ Successfully created all example stories in PRODUCTION!');
    ...
    console.log('Stories are marked as PENDING and will need approval in Sanity Studio (production dataset).');
    
  } catch (error) {
    console.error('❌ Error creating production example stories:', error);
    process.exit(1);
  } finally {
